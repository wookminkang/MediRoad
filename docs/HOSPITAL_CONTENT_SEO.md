...
- **thin 디렉터리 → 주제 권위 페이지로 격상**: 78k 병원 대부분은 주소·전화만 있는 얇은 페이지. 고유 콘텐츠가 붙은 병원만 "리움한방병원 × 디스크 치료" 같은 **병원+질환+지역** 권위 페이지가 됨.
- **오리지널 + 결속**: 병원이 쓴 고유 글을 그 병원 페이지 하위에 두어, 병원의 로컬 신뢰도·내부링크 자산을 물려받음.
- **YMYL 준수**: 의료 콘텐츠는 E-E-A-T(작성자 신뢰도)가 색인의 핵심. 작성자·검수자·출처·날짜 필수.
...
**구조 (답변 우선 — AI 인용 최적화)**
1. 상단 **요약(summary 2~3줄)** = TL;DR → 스니펫·AI 발췌용
2. 질문형 H2/H3 → 한 줄 직답 → 근거 순
3. 병원명·지역·질환 엔티티를 본문에 또박또박 명시
4. 하단 FAQ(3~5개), 참고 출처(refs)

**신뢰도 (필수)**
- 작성자 = 실제 병원/의료진 + 진료과(+가능하면 약력)
- 작성일·수정일 노출
- 의학적 면책 문구
- 과장·단정 금지 (근거 기반 서술)
